    ...
    <div class="w-full h-full flex flex-col place-content-center items-center">
        <form wire:submit="orderStep()" class="w-full h-full flex flex-col place-content-center items-center">
            ...
            <div class="w-5/6 flex justify-end md:px-20">
                <button class="font-archivo text-transparent font-outline-1 text-lg transition ease-in hover:text-black hover:font-outline-none hover:line-through md:text-xl lg:text-2xl xl:text-3xl" wire:click="caseTypeStep()">NEXT</button>
            </div>
            @elseif($currentStep === 2)
            <div class="w-full flex flex-col items-center mb-6">
                <h1 class="font-archivo text-black text-lg mb-2 transition ease-in md:text-xl lg:text-2xl">SELECT CASE TYPE</h1>
                <div class="flex space-x-2">
                    <ul class="w-full flex flex-row gap-1">
                ...
                <div class="font-archivo text-red-600">@error('case_type_id') REQUIRED SELECT CASE TYPE @enderror</div>
            </div>
            ...
            <div class="grid grid-cols-2 p-4 border border-black mb-2">
                ...
                @endif
                @if($selectedCaseType != '')
                <h1 class="font-archivo text-black text-lg text-left transition ease-in md:text-lg lg:text-xl col-span-2">Case Type:</h1>
                <h1 class="font-archivo text-black text-lg mb-2 text-left transition ease-in md:text-lg lg:text-xl">{{ $selectedCaseType->name }}</h1>
                <h1 class="font-archivo text-black text-lg mb-2 text-right transition ease-in md:text-lg lg:text-xl">Rp. {{ $selectedCaseType->price }}</h1>
                @endif
            ...
            <div  class="fixed bottom-0 md:bottom-auto md:right-8 flex flex-col">
                <div id="items-list" class="bg-[#E8F4FA] -md:hidden md:grid grid-cols-2 p-4 border border-black my-2">
                    ...
                    @endif
                    @if($selectedCaseType != '')
                    <h1 class="font-archivo text-black text-sm text-left transition ease-in md:text-md lg:text-md col-span-2">Case Type:</h1>
                    <h1 class="font-archivo text-black text-sm mb-2 text-left transition ease-in md:text-md">{{ $selectedCaseType->name }}</h1>
                    <h1 class="font-archivo text-black text-sm mb-2 text-right transition ease-in md:text-md">Rp. {{ $selectedCaseType->price }}</h1>
                    @endif
                    @if($selectedKeyswitch != '' && $selectedType != '')
